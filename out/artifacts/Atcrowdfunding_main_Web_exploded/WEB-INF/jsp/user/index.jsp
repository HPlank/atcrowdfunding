        ...
        <div class="col-sm-9 col-sm-offset-3 col-md-10 col-md-offset-2 main">
            <div class="panel panel-default">
                ...
                <div class="panel-body">
                    ...
                    <button type="button" class="btn btn-primary" style="float:right;" onclick="window.location.href='${APP_PATH}/user/toAdd.html'"><i class="glyphicon glyphicon-plus"></i> 新增</button>
                    <br>
                    <hr style="clear:both;">
...
<script type="text/javascript">
            ...
            success : function(result){
                ...
                if(result.success){
                    ...
                    $.each(data,function(i,n){
                        ...
                        content+='  <td>';
                        content+='	  <button type="button" onclick="window.location.href=\'${APP_PATH}/user/assignRole.html?id='+n.id+'\'" class="btn btn-success btn-xs"><i class=" glyphicon glyphicon-check"></i></button>';
                        content+='	  <button type="button" onclick="window.location.href=\'${APP_PATH}/user/toUpdate.html?id='+n.id+'\'" class="btn btn-primary btn-xs"><i class=" glyphicon glyphicon-pencil"></i></button>';
    ...
    function deleteUser(id,loginacct) {
        ...
        layer.confirm("点击[确认]删除"+loginacct+"用户！",  {icon: 3, title:'提示'}, function(cindex){
            //确认
            layer.close(cindex);
            $.ajax({
                type:"POST",
                ...
                success : function(result){
                    var loadingIndex = layer.msg('正在删除', {icon: 16});
                    if(result.success){
    ...
    $("#deleteBatchBtn").click(function () {
        ...
        var selectCheckedBox = $("tbody tr td input:checked");
        if(selectCheckedBox.lenth==0){
            layer.msg("至少选择一个用户删除！", {time:1000, icon:5, shift:6});
            layer.close(cindex);
            return false;
        }
        // var idStr = "";

        // $.each(selectCheckedBox,function (i,n) {
            // url?id=5&id=6&id=8
            // if(i!=0){
            //     idStr += "&";
            // }
        ...
        // });
         ...
         $.each(selectCheckedBox,function (i,n) {
            ...
            layer.confirm("点击[确认]删除这些用户！",  {icon: 3, title:'提示'}, function(cindex){
                //确认
                layer.close(cindex);
                $.ajax({
                    type:"POST",
